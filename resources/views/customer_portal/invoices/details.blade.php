...
@section('content') 
    <div class="row">
        ...
        <div class="col-12 col-md-8 text-md-right mb-4">
            ...
            @if($invoice->paid_status != 'PAID' and \App\Models\SystemSetting::isPaypalActive())
            ...
            @endif
            @if($invoice->paid_status != 'PAID' and \App\Models\SystemSetting::isRazorpayActive())
            ...
            @endif
            @if($invoice->paid_status != 'PAID' and \App\Models\SystemSetting::isStripeActive())
            ...
            @endif
            @if($invoice->paid_status != 'PAID' and \App\Models\SystemSetting::isMollieActive())
            ...
            @if($invoice->paid_status != 'PAID' and \App\Models\SystemSetting::isPaymobActive() and $invoice->currency_code == 'EGP')
                @if (\App\Models\SystemSetting::isPaymobCreditCardActive())
                    <a class="btn btn-primary" href="{{ route('customer_portal.invoices.paymob.checkout', [
                        'customer' => $currentCustomer->uid, 
                        'invoice' => $invoice->uid, 
                        'integration_id' => get_system_setting('paymob_credit_card_integration_id'), 
                        'iframe_id' => get_system_setting('paymob_credit_card_iframe_id'),
                ...
                @if (\App\Models\SystemSetting::isPaymobValUActive())
                    <a class="btn btn-primary" href="{{ route('customer_portal.invoices.paymob.checkout', [
                        'customer' => $currentCustomer->uid, 
                        'invoice' => $invoice->uid, 
                        'integration_id' => get_system_setting('paymob_valu_integration_id'), 
                        'iframe_id' => get_system_setting('paymob_valu_iframe_id'),
                        'payment_method' => 'valu'
                    ]) }}">
                        <i class="material-icons">payment</i> 
                        {{ __('messages.pay_with_valu') }}
                    </a>
                @endif
                
                @if (\App\Models\SystemSetting::isPaymobBankInstallmentActive())
                    <a class="btn btn-primary" href="{{ route('customer_portal.invoices.paymob.checkout', [
                        'customer' => $currentCustomer->uid, 
                        'invoice' => $invoice->uid, 
                        'integration_id' => get_system_setting('paymob_bank_installment_integration_id'), 
                        'iframe_id' => get_system_setting('paymob_bank_installment_iframe_id'),
                        'payment_method' => 'bank_installment'
                    ]) }}">
                        <i class="material-icons">payment</i> 
                        {{ __('messages.pay_with_installments') }}
                    </a>
                @endif

                @if (\App\Models\SystemSetting::isPaymobPremiumCardActive())
                    <a class="btn btn-primary" href="{{ route('customer_portal.invoices.paymob.checkout', [
                        'customer' => $currentCustomer->uid, 
                        'invoice' => $invoice->uid, 
                        'integration_id' => get_system_setting('paymob_premium_card_integration_id'), 
                        'iframe_id' => get_system_setting('paymob_premium_card_iframe_id'),
                        'payment_method' => 'premium_card'
                    ]) }}">
                        <i class="material-icons">payment</i> 
                        {{ __('messages.pay_with_premium_card') }}
                    </a>
                @endif
                
                @if (\App\Models\SystemSetting::isPaymobMobileWalletActive())
                    <a class="btn btn-primary" href="{{ route('customer_portal.invoices.paymob.checkout', [
                        'customer' => $currentCustomer->uid, 
                        'invoice' => $invoice->uid, 
                        'integration_id' => get_system_setting('paymob_mobile_wallet_integration_id'), 
                        'iframe_id' => 0,
                        'payment_method' => 'mobile_wallet'
                    ]) }}">
                        <i class="material-icons">payment</i> 
                        {{ __('messages.pay_with_mobile_wallet') }}
                    </a>
                @endif
            @endif
        </div>
        ...
        <div class="col-12">
            @if($invoice->paid_status == 'UNPAID')
                <div class="alert alert-soft-danger d-flex align-items-center" role="alert">
                    ...
                    <div class="text-body"><strong>{{ __('messages.status') }} : </strong> {{ __('messages.' . $invoice->paid_status) }}</div>
                </div>
