@php use App\Factory\StockTypeFactory; @endphp
...
@section('content')
    ...
    <div class="row">
        <div class="col-12 text-right">
            <div class="btn-group mb-2">
                <a href="{{ route('products.edit', ['product' => $product->id, 'company_uid' => $currentCompany->uid]) }}"
                   class="btn btn-light">
                    <i class="material-icons">edit</i>
                    {{ __('messages.edit') }}
                </a>
                <a href="{{ route('products.delete', ['product' => $product->id, 'company_uid' => $currentCompany->uid]) }}"
                   class="btn btn-light delete-confirm">
                    <i class="material-icons">delete</i>
                    {{ __('messages.delete') }}
                </a>
                <div class="btn-group">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a href="#" class="dropdown-item">{{ __('messages.share') }}</a>
                        <a href="#" class="dropdown-item">{{ __('messages.mark_as_active') }}</a>
                        <a href="#" class="dropdown-item">{{ __('messages.mark_as_inactive') }}</a>
                        <hr>
                        <a href="#" class="dropdown-item text-danger">{{ __('messages.archive') }}</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card">
        <div class="row pl-4 pr-4">
            ...
            <div class="col-12 col-md-3 mt-4 mb-4">
                ...
                <p class="mb-1">
                    <strong>{{ __('messages.incoming_stock') }}:</strong> {{ $product->incomingStock->sum("quantity") }}
                    <br>
                </p>
                <p class="mb-1">
                    <strong>{{ __('messages.outgoing_stock') }}:</strong> {{ $product->outgoingStock->sum("quantity") }}
                    <br>
                </p>
                <p class="mb-1">
                    <strong>{{ __('messages.quantity_alarm') }}:</strong> {{ $product->quantity_alarm }} <br>
    ...
    <!-- Tabs Navigation -->
    <nav class="nav nav-pills nav-justified w-100" role="tablist">
        <a href="#variations" class="h6 nav-item nav-link bg-secondary text-white active show" data-toggle="tab"
           role="tab" aria-selected="true">{{ __('messages.variations') }}</a>
        <a href="#incoming_stock" class="h6 nav-item nav-link bg-secondary text-white" data-toggle="tab" role="tab"
           aria-selected="false">{{ __('messages.incoming_stock') }}</a>
        <a href="#outgoing_stock" class="h6 nav-item nav-link bg-secondary text-white" data-toggle="tab" role="tab"
           aria-selected="false">{{ __('messages.outgoing_stock') }}</a>
        <a href="#stock_movements" class="h6 nav-item nav-link bg-secondary text-white" data-toggle="tab" role="tab"
           aria-selected="false">{{ __('messages.stock_movements') }}</a>
        <a href="#customers" class="h6 nav-item nav-link bg-secondary text-white" data-toggle="tab" role="tab"
           aria-selected="false">{{ __('messages.customers') }}</a>
    </nav>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(isset($variation->productVariationColors))
                                            @foreach($variation->productVariationColors as $productVariationColor)
                                                <span
                                                        class="badge badge-primary">{{ $productVariationColor->color }}: {{ $productVariationColor->quantity }}</span>
                                                <br>
                                            @endforeach
                                        @else
                                            N/A
            ...
            <div class="card">
                <div class="table-responsive">
                    <table class="table mb-0 thead-border-top-0 table-striped">
                        ...
                        @if(isset($product->incomingStock) && $product->incomingStock->count() > 0)
                            @php
                                $groupedIncomingStock = $product->incomingStock->groupBy('product_variation_id');
                            @endphp
                            @foreach($groupedIncomingStock as $variationId => $stocks)
                                @php
                                    $currentStock = $stocks->first()->productVariation->base_quantity;
                                @endphp
                                @foreach($stocks as $stock)
                                    @php
                                        $newStock = $currentStock + $stock->quantity;
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $stock->quantity }}</td>
                                        <td>{{ $newStock }}</td>
                                        <td>{{ StockTypeFactory::getStockMovementType($stock->reference_type) }}</td>
                                    </tr>
                                @endforeach
                            @endforeach
                        @else
            ...
            <div class="card">
                <div class="table-responsive">
                    <table class="table mb-0 thead-border-top-0 table-striped">
                        ...
                        @if(isset($product->outgoingStock) && $product->outgoingStock->count() > 0)
                            @php
                                $groupedOutgoingStock = $product->outgoingStock->groupBy('product_variation_id');
                            @endphp
                            @foreach($groupedOutgoingStock as $variationId => $stocks)
                                @php
                                    $currentStock = $stocks->first()->productVariation->base_quantity;
                                @endphp
                                @foreach($stocks as $stock)
            ...
            <div class="card">
                <div class="table-responsive">
                    <table class="table mb-0 thead-border-top-0 table-striped">
                        ...
                        @if(isset($product->stockMovements) && $product->stockMovements->count() > 0)
                            @php
                                $groupedMovements = $product->stockMovements->groupBy('product_variation_id');
                            @endphp
                            @foreach($groupedMovements as $variationId => $movements)
                                @php
                                    $currentStock = $movements->first()->productVariation->base_quantity; // Starting stock for the variation
                                @endphp
                                @foreach($movements as $movement)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $movement->created_at->format($currentCompany->getSetting('date_format')) }}</td>
                                        <td>
                                    <span class="badge {{ $movement->type == 'out' ? 'bg-danger' : 'bg-success' }}">
                                        {{ ucfirst($movement->type) }}
                                    </span>
                                        </td>
                                        <td>
                                    <span class="fw-bold">
                                        <span class="{{ $movement->type == 'out' ? 'text-danger' : 'text-success' }}">
                                            {{ $movement->type == 'out' ? '-' : '+' }}
                                        </span>
                                        <span class="text-dark">{{ $movement->quantity }}</span>
                                    </span>
                                        </td>
                                        <td>{{ $newStock }}</td>
                                        <td>{{ StockTypeFactory::getStockMovementType($movement->reference_type) }}</td>
                                    </tr>
                                @endforeach
                            @endforeach
                        @else
